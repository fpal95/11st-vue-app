    ...
    <div class="banner">
      <a :href="navigations.exhibitions.href">
        <img
          ...
          :alt="navigations.exhibitions.alt" />
      </a>
    </div>
  </nav>
  <div
...
export default {
  ...
  methods: {
    ...
    toggleGroup() {
      const pascalCaseName = _upperFirst()
  ...
  .container {
    ...
    .group {
      ...
      &.major-services {
        .group__list {
          display: flex;
          flex-wrap: wrap; // 줄바꿈하겠다. 디폴트는 nowrap
          li {
            width: 50%; // 2개의 아이템이 각각 50%씩 차지
            height: 50px;
            a {
              padding-left: 25px;
            }
            &:hover { // &는 상위 선택자. 즉, li의 hover일 때!!
              background-color: #fafafa;
              color: #ff5534;
              a {
                color: #ff5534;
